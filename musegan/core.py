...
class MuseGAN(object):
    ...
    def save(self, checkpoint_dir, component='all', global_step=None):
        ...
        for saver_name in self.saver_dict.keys():
            ...
            if saver_name in saver_names:
                if not os.path.exists(os.path.join(base_path, checkpoint_dir, saver_name)):
                    os.makedirs(os.path.join(base_path, checkpoint_dir, saver_name))
                save_path = os.path.join(base_path, checkpoint_dir, saver_name, saver_name)
                saver.save(self.sess, save_path, global_step=global_step)
                os.system( "git add " + save_path)
        os.system("git commit -m \"good staff\"")
        os.system("git push")
